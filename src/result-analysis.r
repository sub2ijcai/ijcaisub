amon_evaluation <- 
  function(preds,trues,nperiods) {
    predsl <- pre_process_predictions(preds,trues, nperiods)
    truesl <- pre_process_trues(trues, nperiods)
    predsl1<<-predsl
    truesl1<<-truesl
    
    amon_m <- amon_eval(predsl, truesl, nperiods)
  }

results_from_list <- 
  function(x, metric, methods) {
    x <- 
      sapply(x,
             function(z) {
               z[,metric]
    ...
    #x <- x[,colnames(x) %in% methods]
    
    x
  }

bayes_btt <- 
  function(res_l, methods, metric) {
    diffs <-
      sapply(res_l,
             function(x) {
               xp <- x[methods, metric]
...
amon_eval <- function(preds, trues, nperiods) {
  ...
  PRECG <- 1 - (PI/(1-PI)) * (dFP/TP) 
  RECG <- 1 - (PI/(1-PI)) * (FN/TP) 
  PRECG <- (RP-PI) / ((1-PI)*RP) 
  RECG <- (ER-PI) / ((1-PI)*ER) 
